...
func runBuild(cmd *command) (err error) {
	...
	case "darwin":
		...
		_, err = goIOSBuild(pkg, buildBundleID)
		if err != nil {
			return err
		}
	}

	// if !nmpkgs["github.com/ZedContinuum/mobile/app"] {
	// 	return fmt.Errorf(`%s does not import "github.com/ZedContinuum/mobile/app"`, pkg.ImportPath)
	// }
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/ZedContinuum/mobile/app": true}, nil
	}
	r, w := io.Pipe()
