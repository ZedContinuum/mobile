...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/ZedContinuum/mobile/cmd/gomobile
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "install", "-x", "-n", "github.com/ZedContinuum/mobile/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/ZedContinuum/mobile/app").CombinedOutput()
	if err != nil {
