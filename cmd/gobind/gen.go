...
func genPkg(p *types.Package, allPkg []*types.Package, classes []*java.Class) {
	...
	case "java":
		...
		if p == nil {
			p, err := build.Default.Import("github.com/ZedContinuum/mobile/bind", ".", build.ImportComment)
			if err != nil {
				errorf(`"github.com/ZedContinuum/mobile/bind" is not found; run go get github.com/ZedContinuum/mobile/bind: %v`)
				return
			}
			repo := filepath.Clean(filepath.Join(p.Dir, "..")) // github.com/ZedContinuum/mobile directory.
